...
namespace HachimitsuYogurt
{
    class Regulation
    {
        ...
        public List<int> MovableKomaMappingBoard(int pos, MovablePattern movablepattern, int dummy)
        {
            ...
            // pos から、棋譜上で移動パターンをマッピングする
            foreach (var s in movablepattern.pattern0)
            {
                int x = pos / 10;
                int y = pos % (x * 10);
            ...
            foreach (var s in movablepattern.pattern1)
            {
                int x = pos / 10;
                int y = pos % (x * 10);
            ...
            foreach (var s in movablepattern.pattern2)
            {
                int x = pos / 10;
                int y = pos % (x * 10);
            ...
            foreach (var s in movablepattern.pattern3)
            {
                int x = pos / 10;
                int y = pos % (x * 10);
